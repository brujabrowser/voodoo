// Golden test for the mojo.* message-pipe bindings -- mirrors
 // tests/system/message_pipe_roundtrip.cc's CreateAndClose / WriteReadRaw
 // cases, but drives them from JS on the WASMv8bindings facade.
#include <cassert>
#include <cstdio>
...
int main() {
  v8::Isolate* isolate = v8::Isolate::New();
  {
    ...
    assert(RunNumber(context,
                     "mojo.writeMessageRaw(pipe.handle0, 'hello kernel')") ==
           MOJO_RESULT_OK);
    Run(context, "globalThis.msg = mojo.readMessageRaw(pipe.handle1)");
    assert(RunNumber(context, "msg.result") == MOJO_RESULT_OK);
    assert(RunBool(context, "msg.payload === 'hello kernel'"));
    assert(RunNumber(context, "msg.handleCount") == 0);
    ...
    std::printf("wck_bindings_message_pipe_test: ok\n");
  }
  isolate->Dispose();
  return 0;
}
