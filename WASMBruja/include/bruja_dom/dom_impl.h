// A real, reusable implementation of the interfaces generated from
// examples/dom/dom.bruja -- extracted from tests/golden/dom_roundtrip.cc
// (which proved brujac's codegen against these exact classes) so any
// embedder (e.g. WASMBlinker's LocalFrameImpl) can install a real
// document/window rather than re-deriving one. Header-only, matching this
// family's convention for template-heavy code.
//
// Improvements over the golden test's own copy (which deliberately left
// these as documented stubs -- proving the grammar/binding plumbing, not a
// full engine): Document/Element's GetElementById/GetElementsByTagName/
// GetElementsByClassName/QuerySelector/QuerySelectorAll are real tree
// walks now (a minimal single-selector matcher: `#id`, `.class`, or a bare
// tag name -- no combinators, no comma lists, matching this family's
// existing "modeled on, not a literal port" convention elsewhere), and
// Document::CreateElement covers every concrete HTMLXxxElement this file
// defines instead of just three.
//
// Still-open limitation carried over unchanged (see the golden test's own
// header comment): Node::AppendChild does not wire the child's reverse
// `parent` pointer automatically -- the abstract `Node*` interface has no
// way to reach a concrete impl's `parent` field polymorphically. Code that
// builds a tree from a known concrete type (see WASMBlinker's
// dom_bindings.cc) wires `parent` by hand at construction time; a
// `Node`/`Element` created and appended purely from JS
// (`document.createElement(...)` then `.appendChild(...)`) will NOT have
// `parentNode` reflect that until this is solved for real (would need a
// common non-templated base carrying `parent`, or a virtual "AdoptChild"
// hook every concrete leaf implements).
#ifndef BRUJA_DOM_IMPL_H_
#define BRUJA_DOM_IMPL_H_
...
class HTMLAnchorElementImpl : public HTMLElementImplT<HTMLAnchorElement> {
 public:
  ...
  // Same reason dom_v8_impl.h's identical method exists -- Href()/
  // Target() are separate fields the generic attributes_ map alone
  // doesn't populate.
  void SetAttribute(const std::string& name, const std::string& value) override {
...
class HTMLImageElementImpl : public HTMLElementImplT<HTMLImageElement> {
 public:
  ...
  // Not part of the generated interface -- see dom_v8_impl.h's identical
  // method for why (readonly attributes have no SetX; this is the
  // quickjs-backend twin of that same addition).
...
class HTMLInputElementImpl : public HTMLElementImplT<HTMLInputElement> {
 public:
  ...
  // Same reason dom_v8_impl.h's identical method exists.
  void SetAttribute(const std::string& name, const std::string& value) override {
